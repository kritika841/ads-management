...
export function DashboardClient({ profile, ads, campaigns, products, profiles, availableTags, editorWorkloads, initialQueue }: { profile: Profile; ads: AdWithRelations[]; campaigns: Campaign[]; products: Product[]; profiles: Profile[]; availableTags: string[]; editorWorkloads: Record<string, number>; initialQueue: QueueKey }) {
  ...
  return <main className="page-container">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h1 className="text-2xl font-semibold text-slate-950">Creative library</h1><p className="mt-1 text-sm text-slate-500">Your work, organized by what needs attention next.</p></div>{canCreate ? <Button onClick={() => openCreatorForm()}><Plus className="size-4" aria-hidden />Add creative</Button> : null}</div>

    <div className="mt-6 overflow-x-auto pb-1"><div className="inline-flex min-w-max rounded-md border border-border bg-white p-1 shadow-sm">{queueOptions.map((option) => <button key={option.key} className={cn("flex h-9 items-center gap-2 rounded px-3 text-sm font-medium transition", queue === option.key ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-50")} onClick={() => selectQueue(option.key)}>{option.label}<span className={cn("rounded-full px-1.5 py-0.5 text-[10px]", queue === option.key ? "bg-white/15" : "bg-slate-100 text-slate-500")}>{ads.filter((ad) => matchesQueue(ad, option.key)).length}</span></button>)}</div></div>

    <section className="panel mt-4 p-3"><div className="flex flex-col gap-2 lg:flex-row"><div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" aria-hidden /><Input ref={searchRef} className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search ads, scripts, people, products, or tags" /></div><Button variant={filtersOpen || filtersActive ? "primary" : "secondary"} onClick={() => setFiltersOpen((current) => !current)}><Filter className="size-4" aria-hidden />Filters{filtersActive ? <span className="size-1.5 rounded-full bg-current" /> : null}</Button><div className="flex rounded-md border border-border bg-slate-50 p-1"><Button size="icon" variant={view === "grid" ? "secondary" : "ghost"} className="size-8" title="Grid view" onClick={() => setView("grid")}><Grid2X2 className="size-4" aria-hidden /></Button><Button size="icon" variant={view === "table" ? "secondary" : "ghost"} className="size-8" title="Table view" onClick={() => setView("table")}><Table2 className="size-4" aria-hidden /></Button></div></div>{filtersOpen ? <div className="mt-3 border-t border-border pt-3"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><FilterSelect label="Status" value={stage} onChange={setStage} options={productionStages.map((item) => ({ value: item, label: productionStageLabels[item] }))} />{profile.role === "admin" || profile.role === "manager" ? <><FilterSelect label="Editor" value={editor} onChange={setEditor} options={editors.map((item) => ({ value: item.id, label: item.name }))} /><FilterSelect label="Creator" value={creator} onChange={setCreator} options={creators.map((item) => ({ value: item.id, label: item.name }))} /></> : null}<FilterSelect label="Campaign" value={campaign} onChange={setCampaign} options={campaigns.map((item) => ({ value: item.id, label: item.name }))} /><FilterSelect label="Product" value={product} onChange={setProduct} options={products.map((item) => ({ value: item.id, label: item.name }))} /><FilterSelect label="Platform" value={platform} onChange={setPlatform} options={platforms.map((item) => ({ value: item, label: item }))} /><FilterSelect label="Tag" value={tag} onChange={setTag} options={availableTags.map((item) => ({ value: item, label: `#${item}` }))} /><FilterSelect label="Deadline" value={deadline} onChange={setDeadline} options={[{ value: "overdue", label: "Overdue" }, { value: "today", label: "Due today" }, { value: "soon", label: "Due in 3 days" }]} /><FilterSelect label="Sort" value={sort} onChange={setSort} options={[{ value: "deadline", label: "Deadline first" }, { value: "waiting", label: "Waiting longest" }, { value: "oldest", label: "Oldest created" }]} /></div>{filtersActive ? <div className="mt-3 flex justify-end"><Button size="sm" variant="ghost" onClick={clearFilters}><X className="size-3.5" aria-hidden />Clear filters</Button></div> : null}</div> : null}</section>

    {actionMessage ? <div className={cn("mt-3 flex items-center justify-between rounded-md border px-3 py-2 text-sm", actionMessage.tone === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800")} role={actionMessage.tone === "error" ? "alert" : "status"}><span>{actionMessage.text}</span><button title="Dismiss" onClick={() => setActionMessage(null)}><X className="size-4" aria-hidden /></button></div> : null}

    <div className="mt-4 flex items-center justify-between"><p className="text-sm text-slate-500"><span className="font-medium text-slate-800">{filteredAds.length}</span> items</p></div>
    ...
    {formOpen ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-0 backdrop-blur-[2px] sm:p-6" role="dialog" aria-modal="true" aria-labelledby="creator-form-title"><section className="mx-auto min-h-full w-full bg-white shadow-float sm:min-h-0 sm:max-w-5xl sm:rounded-lg"><div className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-border bg-white px-5 sm:rounded-t-lg"><div><h2 id="creator-form-title" className="text-lg font-semibold text-slate-950">{editingAd ? "Update creative" : "Add creative"}</h2><p className="text-xs text-slate-500">Set the current preparation status and save.</p></div><Button size="icon" variant="ghost" title="Close" onClick={() => { setFormOpen(false); setEditingAd(null); }}><X className="size-5" aria-hidden /></Button></div><div className="p-5"><CreatorItemForm profile={profile} creators={creators} editors={editors} campaigns={campaigns.filter((item) => item.active)} products={products.filter((item) => item.active)} initialAd={editingAd} availableTags={availableTags} editorWorkloads={editorWorkloads} onSaved={() => { setFormOpen(false); setEditingAd(null); router.refresh(); }} /></div></section></div> : null}

    {cancelAd ? <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="changes-title"><section className="w-full max-w-lg rounded-lg bg-white shadow-float"><div className="flex items-start justify-between border-b border-border px-5 py-4"><div><h2 id="changes-title" className="text-lg font-semibold text-slate-950">Request changes</h2><p className="mt-1 text-sm text-slate-500">Tell the assigned editor exactly what must change.</p></div><Button size="icon" variant="ghost" className="size-9" title="Close" onClick={() => setCancelAd(null)}><X className="size-5" aria-hidden /></Button></div><div className="p-5"><Textarea className="min-h-32" value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} placeholder="Required changes" /></div><div className="flex justify-end gap-2 border-t border-border px-5 py-4"><Button variant="secondary" onClick={() => setCancelAd(null)}>Keep in review</Button><Button variant="danger" disabled={isPending || !cancelReason.trim()} onClick={() => decide(cancelAd, "request_changes", cancelReason.trim())}>{isPending ? <Loader2 className="size-4 animate-spin" aria-hidden /> : <X className="size-4" aria-hidden />}Send changes</Button></div></section></div> : null}
...
function WorkflowCard({ ad, profile, editors, editorWorkloads, pending, onPreview, onEdit, onApprove, onRequestChanges, onAssignEditor }: { ad: AdWithRelations; profile: Profile; editors: Profile[]; editorWorkloads: Record<string, number>; pending: boolean; onPreview: () => void; onEdit: () => void; onApprove: () => void; onRequestChanges: () => void; onAssignEditor: (editorId: string, deadline: string) => void }) {
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-border bg-white shadow-soft">
      {canPreview ? (
        <button className="relative block aspect-video w-full overflow-hidden bg-slate-950" onClick={onPreview} aria-label={`Preview ${ad.name}`}>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><h2 className="truncate text-[15px] font-semibold text-slate-950">{ad.name}</h2><p className="mt-0.5 truncate text-xs text-slate-500">{ad.campaign?.name ?? "No campaign"}</p></div>
          <div className="flex gap-0.5">{canDeleteAd(profile.role) ? <DeleteAdButton adId={ad.id} adName={ad.name} compact /> : null}{canPreview ? <button className="inline-flex size-8 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100" title="Preview" onClick={onPreview}><Eye className="size-4" aria-hidden /></button> : null}</div>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between gap-3 text-xs"><span className="font-medium text-slate-500" suppressHydrationWarning>{workflowWaitingLabel(ad.workflow_status_changed_at)}</span><Deadline deadline={ad.deadline} status={ad.status} /></div>
        {canFinalReview ? (
          ...
          <Link href={`/ads/${ad.id}`} className="mt-3 inline-flex h-9 w-full items-center justify-center gap-2 rounded-md border border-border bg-white text-sm font-medium text-slate-800 hover:bg-slate-50">{actionLabel}<ArrowRight className="size-3.5" aria-hidden /></Link>
        )}
      </div>
    </article>
  );
...
function MediaPlaceholder({ label }: { label: string }) {
  return <span className="flex size-full flex-col items-center justify-center gap-2 text-sm text-slate-400"><Video className="size-6" aria-hidden />{label}</span>;
}
...
function WorkflowTable({ ads, profile, pendingId, onApprove, onRequestChanges }: { ads: AdWithRelations[]; profile: Profile; pendingId: string | null; onApprove: (ad: AdWithRelations) => void; onRequestChanges: (ad: AdWithRelations) => void }) {
  const reviewer = profile.role === "admin" || profile.role === "manager";
  return <section className="panel mt-3 overflow-x-auto"><table className="w-full min-w-[920px] text-left text-sm"><thead className="border-b border-border bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Creative</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Creator</th><th className="px-4 py-3">Editor</th><th className="px-4 py-3">Waiting</th><th className="px-4 py-3" /></tr></thead><tbody className="divide-y divide-border">{ads.map((ad) => { const reviewable = reviewer && (ad.production_stage === "creator_review" || ad.production_stage === "final_review"); return <tr key={ad.id} className="hover:bg-slate-50"><td className="px-4 py-3"><Link href={`/ads/${ad.id}`} className="font-medium text-slate-900 hover:text-primary">{ad.name}</Link><p className="text-xs text-slate-500">{ad.campaign?.name}</p></td><td className="px-4 py-3"><ProductionStageBadge stage={ad.production_stage} className="bg-slate-100 text-slate-700 shadow-none" /></td><td className="px-4 py-3">{ad.creator?.name ?? "Unassigned"}</td><td className="px-4 py-3">{ad.editor?.name ?? "Unassigned"}</td><td className="px-4 py-3 text-slate-500" suppressHydrationWarning>{workflowWaitingLabel(ad.workflow_status_changed_at)}</td><td className="px-4 py-3"><div className="flex justify-end gap-2">{reviewable ? <><Button size="sm" disabled={pendingId === ad.id} onClick={() => onApprove(ad)}>Approve</Button><Button size="sm" variant="secondary" onClick={() => onRequestChanges(ad)}>Changes</Button></> : <Link href={`/ads/${ad.id}`} className="inline-flex h-9 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium text-slate-700">Open<ArrowRight className="size-3.5" aria-hidden /></Link>}{canDeleteAd(profile.role) ? <DeleteAdButton adId={ad.id} adName={ad.name} compact /> : null}</div></td></tr>; })}</tbody></table></section>;
}

function Person({ label, person }: { label: string; person: AdWithRelations["creator"] }) { return <div className="flex min-w-0 items-center gap-2"><Avatar className="size-7" name={person?.name ?? "Unassigned"} src={person?.avatar_url} /><div className="min-w-0"><p className="text-[10px] font-medium uppercase text-slate-400">{label}</p><p className="truncate text-xs font-medium text-slate-700">{person?.name ?? "Unassigned"}</p></div></div>; }
function Deadline({ deadline, status }: { deadline: string | null; status: AdStatus }) { if (!deadline) return <span className="text-slate-400">No deadline</span>; const days = dateOnlyDaysFromToday(deadline); const active = status !== "approved" && status !== "published"; return <span className={cn("inline-flex items-center gap-1", active && days < 0 ? "text-rose-600" : "text-slate-400")}><CalendarClock className="size-3.5" aria-hidden />{active && days < 0 ? `${Math.abs(days)}d overdue` : formatDateOnly(deadline)}</span>; }
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: { value: string; label: string }[] }) { return <label className="space-y-1"><span className="text-xs font-medium text-slate-500">{label}</span><Select value={value} onChange={(event) => onChange(event.target.value)}><option value="all">{label === "Sort" ? "Recently updated" : `All ${label.toLowerCase()}`}</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></label>; }
function EmptyQueue({ canCreate, onCreate }: { canCreate: boolean; onCreate: () => void }) { return <div className="mt-6 flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white px-6 text-center"><ListFilter className="size-6 text-slate-300" aria-hidden /><h2 className="mt-3 text-base font-semibold text-slate-900">Nothing in this queue</h2><p className="mt-1 text-sm text-slate-500">Items will appear here when they reach this status.</p>{canCreate ? <Button className="mt-4" onClick={onCreate}><Plus className="size-4" aria-hidden />Add creative</Button> : null}</div>; }
