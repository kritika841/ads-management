...
function MediaPlaceholder({ label }: { label: string }) {
  return <span className="flex size-full flex-col items-center justify-center gap-2 text-sm text-muted-foreground"><Video className="size-6" aria-hidden />{label}</span>;
}
...
function EmptyQueue({ canCreate, onCreate }: { canCreate: boolean; onCreate: () => void }) { return <div className="mt-6 flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card px-6 text-center"><ListFilter className="size-6 text-border" aria-hidden /><h2 className="mt-3 text-base font-semibold text-foreground">Nothing in this queue</h2><p className="mt-1 text-sm text-muted-foreground">Items will appear here when they reach this status.</p>{canCreate ? <Button className="mt-4" onClick={onCreate}><Plus className="size-4" aria-hidden />Add creative</Button> : null}</div>; }
