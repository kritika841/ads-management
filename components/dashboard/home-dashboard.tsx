...
export function HomeDashboard({ model }: { model: DashboardSummaryModel }) {
  return <main className="page-container"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h1 className="text-2xl font-semibold text-foreground">Home</h1><p className="mt-1 text-sm text-muted-foreground">A focused view of what needs attention today.</p></div><Link href="/library" className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-md border border-border bg-card px-4 text-sm font-medium text-foreground shadow-sm hover:bg-muted">Open creative library<ArrowRight className="size-4" aria-hidden /></Link></div><DashboardToday model={model} onSelectQueue={(queue) => { window.location.href = `/library?queue=${queue}`; }} /></main>;
}
