...
export function AdPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/50 p-0 backdrop-blur-[2px] sm:p-6" role="dialog" aria-modal="true" aria-labelledby="preview-title">
      <div className="mx-auto flex h-full max-w-7xl flex-col overflow-hidden bg-white shadow-float sm:rounded-lg">
        <header className="flex min-h-16 items-center justify-between gap-4 border-b border-border px-4 py-3 sm:px-5">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h2 id="preview-title" className="truncate text-lg font-semibold text-slate-950">{ad.name}</h2>
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className={cn(
                "inline-flex size-9 items-center justify-center gap-2 rounded-md bg-slate-950 text-sm font-medium text-white transition hover:bg-slate-800 sm:w-auto sm:px-3"
              )}
            >
        ...
        <div className="grid min-h-0 flex-1 overflow-y-auto lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.8fr)] lg:overflow-hidden">
          <div className="min-h-[300px] bg-slate-950 lg:min-h-0">
            {ad.preview_url ? (
              ...
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center gap-2 text-sm text-slate-400"><Video className="size-7" aria-hidden />Preview unavailable</div>
            )}
          </div>
          <aside className="min-h-0 border-t border-border p-5 lg:overflow-y-auto lg:border-l lg:border-t-0 lg:p-6">
            <div className="flex items-center justify-between gap-3"><h3 className="section-heading">Script</h3><span className="text-xs text-slate-400">{ad.script_text?.split(/\s+/).filter(Boolean).length ?? 0} words</span></div>
            <div
