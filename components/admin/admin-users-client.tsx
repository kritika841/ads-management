  ...
  return (
    <main className="page-container">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><h1 className="text-2xl font-semibold text-slate-950">People</h1><p className="mt-1 text-sm text-slate-500">Manage access, roles, passwords, and team performance.</p></div>
      ...
      <section className="panel mt-4 overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-border p-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-sm"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" aria-hidden /><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people" /></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[960px] text-left text-sm">
            <thead className="border-b border-border bg-slate-50/80 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Person</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Ads</th><th className="px-4 py-3">Approved</th><th className="px-4 py-3">Approval rate</th><th className="px-4 py-3">Avg approval</th><th className="px-4 py-3" /></tr></thead>
            <tbody className="divide-y divide-border">
              {filteredProfiles.map((profile) => {
                const userStats = stats.get(profile.id);
                return (
                  <tr key={profile.id} className={cn("transition hover:bg-slate-50/80", !profile.active && "bg-slate-50/60")}>
                    <td className="px-4 py-3"><div className="flex items-center gap-3"><Avatar name={profile.name} src={profile.avatar_url} /><div className="min-w-0"><div className="font-medium text-slate-950">{profile.name}</div><div className="text-xs text-slate-500">{profile.email}</div></div></div></td>
                    <td className="px-4 py-3"><span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">{roleLabel(profile.role)}</span></td>
                    <td className="px-4 py-3"><span className={cn("inline-flex items-center gap-1.5 text-xs font-medium", profile.active ? "text-emerald-700" : "text-slate-400")}><span className={cn("size-1.5 rounded-full", profile.active ? "bg-emerald-500" : "bg-slate-300")} />{profile.active ? "Active" : "Inactive"}</span></td>
                    <td className="px-4 py-3 text-slate-700">{userStats?.submitted ?? 0}</td>
                    <td className="px-4 py-3 text-slate-700">{userStats?.approved ?? 0}</td>
                    <td className="px-4 py-3"><span className="font-medium text-slate-800">{userStats?.approvalRate ?? 0}%</span></td>
                    <td className="px-4 py-3 text-slate-500">{formatDurationHours(userStats?.avgApproval ?? null)}</td>
                    <td className="px-4 py-3"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" className="size-9" title="Edit user" onClick={() => open(profile)}><Pencil className="size-4" aria-hidden /></Button><Button size="icon" variant="ghost" className="size-9 text-rose-600 hover:bg-rose-50 hover:text-rose-700" title={profile.id === currentProfileId ? "You cannot deactivate yourself" : "Deactivate user"} disabled={!profile.active || profile.id === currentProfileId} onClick={() => deactivate(profile)}><Power className="size-4" aria-hidden /></Button></div></td>
                  </tr>
                );
              })}
            </tbody>
        ...
        {!filteredProfiles.length ? <div className="flex min-h-48 flex-col items-center justify-center text-center"><Users className="size-6 text-slate-300" aria-hidden /><p className="mt-3 text-sm font-medium text-slate-700">No people found</p><p className="mt-1 text-xs text-slate-500">Try another search or role.</p></div> : null}
      </section>

      {modalOpen ? (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-0 backdrop-blur-[2px] sm:p-6" role="dialog" aria-modal="true" aria-labelledby="user-modal-title">
          <section className="mx-auto min-h-full w-full bg-white shadow-float sm:min-h-0 sm:max-w-xl sm:rounded-lg">
            <div className="flex h-16 items-center justify-between border-b border-border px-5"><div><h2 id="user-modal-title" className="text-lg font-semibold text-slate-950">{editing ? "Edit user" : "Add approved user"}</h2><p className="text-xs text-slate-500">{editing ? "Update access and profile details" : "Create login credentials manually"}</p></div><Button size="icon" variant="ghost" title="Close" onClick={close}><X className="size-5" aria-hidden /></Button></div>
            <div className="space-y-4 p-5">
              ...
              <div className="rounded-md border border-border bg-slate-50 p-3"><Field label={editing ? "New password" : "Initial password"} hint={editing ? "Optional. Leave blank to keep the current password." : undefined}><div className="flex gap-2"><Input value={newPassword} onChange={(event) => { setNewPassword(event.target.value); setCopied(false); }} type="text" autoComplete="new-password" placeholder="Minimum 8 characters" /><Button size="icon" variant="secondary" title="Copy password" disabled={!newPassword} onClick={copyPassword}>{copied ? <Check className="size-4 text-emerald-600" aria-hidden /> : <Copy className="size-4" aria-hidden />}</Button><Button size="icon" variant="secondary" title="Generate password" onClick={() => { setNewPassword(generatePassword()); setCopied(false); }}><RefreshCw className="size-4" aria-hidden /></Button></div></Field></div>
...
function Metric({ label, value, tone }: { label: string; value: number; tone?: "emerald" }) {
  return <div className="bg-white px-4 py-4"><p className="text-xs font-medium text-slate-500">{label}</p><p className={cn("mt-1 text-2xl font-semibold text-slate-950", tone === "emerald" && "text-emerald-700")}>{value}</p></div>;
}
