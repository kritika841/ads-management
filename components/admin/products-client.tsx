    ...
    <main className="page-container">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">The product catalog creatives get made for.</p>
        </div>
        ...
        {filteredProducts.length ? (
          <div className="grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-3">
            {filteredProducts.map((product) => (
              <div key={product.id} className={cn("flex items-center gap-3 bg-card p-4", !product.active && "bg-muted/60")}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-muted-foreground">{product.sku || "No SKU"}</p>
                  <span className={cn("mt-1 inline-flex items-center gap-1.5 text-xs font-medium", product.active ? "text-success" : "text-muted-foreground")}>
                    <span className={cn("size-1.5 rounded-full", product.active ? "bg-success" : "bg-border")} />
          ...
          <div className="flex min-h-48 flex-col items-center justify-center text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Add a product so creatives can be linked to it.</p>
          </div>
        )}
      </section>
          ...
          <section className="mx-auto min-h-full w-full bg-card shadow-float sm:min-h-0 sm:max-w-lg sm:rounded-xl">
            <div className="flex h-16 items-center justify-between border-b border-border px-5">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{editing ? "Update the product details" : "Add a product to the catalog"}</p>
              </div>
