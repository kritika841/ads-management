    ...
    <div className="space-y-6">
      ...
      {requiresRawFootage ? (
        <section className="rounded-md border border-teal-200 bg-teal-50/60 p-4">
          <h3 className="text-sm font-semibold text-slate-950">{isHandoff ? "Editor handoff" : "Raw footage"}</h3>
          <p className="mt-1 text-sm text-slate-600">{isHandoff ? "Assigning an editor gives them private access to this script and raw footage. You can also leave it unassigned and assign one later." : "Add the raw footage folder link now that the shoot is complete."}</p>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <Field label="Raw footage folder" hint="Required."><div className="relative"><Link2 className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" aria-hidden /><Input className="pl-9" value={rawFootageUrl} onChange={(event) => setRawFootageUrl(event.target.value)} placeholder="https://drive.google.com/drive/folders/..." required /></div></Field>
      ...
      <Field label="Platforms"><div className="grid grid-cols-2 gap-2 lg:grid-cols-4">{platforms.map((platform) => { const selected = selectedPlatforms.includes(platform); return <button key={platform} type="button" aria-pressed={selected} className={`flex h-10 items-center justify-between rounded-md border px-3 text-sm ${selected ? "border-primary bg-teal-50 text-teal-800" : "border-border bg-white text-slate-600"}`} onClick={() => setSelectedPlatforms((current) => selected ? current.filter((item) => item !== platform) : [...current, platform])}><span>{platform}</span>{selected ? <Check className="size-4" aria-hidden /> : null}</button>; })}</div></Field>

      <Field label="Tags"><div className="flex flex-wrap gap-2">{tagOptions.map((tag) => <button key={tag} type="button" className={`rounded-full border px-3 py-1.5 text-xs ${selectedTags.includes(tag) ? "border-primary bg-teal-50 text-teal-800" : "border-border text-slate-600"}`} onClick={() => setSelectedTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag])}>#{tag}</button>)}</div><div className="mt-2 flex gap-2"><div className="relative flex-1"><Tags className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" aria-hidden /><Input className="pl-9" value={tagDraft} onChange={(event) => setTagDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTag(); } }} placeholder="Add tag" /></div><Button variant="secondary" disabled={!tagDraft.trim()} onClick={addTag}><Plus className="size-4" aria-hidden />Add</Button></div></Field>

      {message ? <p className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800" role="alert">{message}</p> : null}
