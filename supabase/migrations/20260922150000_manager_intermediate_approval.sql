-- Update final_review_ad_atomic to allow manager approval when allow_manager_final_approval is false.
-- Instead of raising an exception, it intercepts manager approval, keeping production_stage = 'final_review'
-- and advancing approval_stage to 'admin_final' so it moves to Administrator for final approval.
...
begin
  ...
  if p_decision = 'approve' then
    ...
    if p_target = 'creator' then
      ...
      activity_action := 'final_changes_requested_to_editor';
...
revoke all on function public.final_review_ad_atomic(uuid, uuid, text, text, text) from public, anon, authenticated;
grant execute on function public.final_review_ad_atomic(uuid, uuid, text, text, text) to service_role;
