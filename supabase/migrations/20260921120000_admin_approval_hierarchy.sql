...
add column if not exists allow_manager_final_approval boolean not null default true;
...
begin
  ...
  if p_decision = 'approve' and actor_role = 'manager' and not coalesce(v_allow_manager, true) then
    raise exception 'Final approval is restricted to administrators';
  end if;
