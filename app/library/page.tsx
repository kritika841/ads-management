...
export default async function LibraryPage({ searchParams }: { searchParams: Promise<{ queue?: string | string[] }> }) {
  ...
  return <AppShell profile={profile} notifications={notifications}><DashboardClient profile={profile} ads={ads} campaigns={campaigns} products={products} profiles={profiles} availableTags={tags} editorWorkloads={editorWorkloads} initialQueue={initialQueue} /></AppShell>;
}
