...
export function groupSegmentMatchesByAd(matches, limit = 15) {
  ...
  for (const match of matches || []) {
    ...
    const current = bestByAd.get(normalized.ad_id);
    if (!current || normalized.similarity > current.similarity) {
      bestByAd.set(normalized.ad_id, normalized);
    }
  }

  return Array.from(bestByAd.values())
